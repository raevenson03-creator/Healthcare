...
import { getAppointments, getLabReports, getMedications, getVitals } from '@/services/clinical.service';
  ...
  return (
    <Screen>
      ...
      {upcoming.length === 0 ? (
        ...
        upcoming.map((appt, i) => (
          <FadeInView key={appt.id} delay={100 + i * 50}>
            <Card
              onPress={() =>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing.md }}>
        {(vitals.data ?? []).slice(0, 4).map((v, i) => (
          <FadeInView key={v.id} delay={360 + i * 40} style={{ flexBasis: '47%', flexGrow: 1 }}>
            <Card>
              <Text variant="caption" tone="muted">
                {v.label}
              </Text>
      ...
      <FadeInView delay={440}>
        <View style={{ gap: theme.spacing.sm }}>
          ...
          <Button
            ...
            variant="secondary"
            onPress={() => navigation.navigate('Records', { screen: 'RecordsOverview' })}
          />
        </View>
      </FadeInView>
    </Screen>
